...
bool test_graph()
{
        ...
        VertexMap::const_iterator it2;

        for(it2 = g->getVertexMap().begin(); it2 != g->getVertexMap().end(); ++it2)
                std::cout << it2->first << std::endl;

        //Add an edge
        Edge e1 = g->addEdge(g->getVertex("George"),g->getVertex("Spyros"));
        ...
        g->getVertexMap()[g->getGraph()[nsource].name];
        ...
        Edge e4 = g->addEdge(g->getVertex("Spyros"),g->getVertex("George"));

        std::cout << "These are the leafnodes of the graph" << endl;

        std::vector< Vertex> leafnodes = g->getLeafNodes();

        if(leafnodes.size() > 0)
                for(std::vector< Vertex>::iterator it = leafnodes.begin(); it != leafnodes.end(); ++it)
        ...
        //g = Network::generateRandomNetwork();
